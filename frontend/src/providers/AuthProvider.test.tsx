...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <button data-testid="register-btn" onClick={() => login('u', 'p')}>Register</button>
...
describe('AuthProvider', () => {
  ...
  it('shows loading state while restoring token', () => {
    ...
    vi.spyOn(authService, 'getMe').mockImplementation(() => new Promise(() => {}));
    renderWithProvider();
  ...
  it('clears token when getMe fails', async () => {
    ...
    vi.spyOn(authService, 'getMe').mockRejectedValue(new Error('Token expired'));
  ...
  it('login sets user and token', async () => {
    ...
    vi.spyOn(authService, 'login').mockResolvedValue({
      ...
      token_type: 'bearer',
      expires_in: 86400,
      user: { id: '2', username: 'newuser', email: 'n@t.com', created_at: '2024-01-01', is_active: true },
    });
