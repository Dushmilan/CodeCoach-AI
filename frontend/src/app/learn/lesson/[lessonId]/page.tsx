...
import { ArrowLeft, ArrowRight, BookOpen, Check, Code } from "lucide-react";
...
export default function LessonPage() {
  ...
  const [language, setLanguage] = useState<Language>(
    (lesson?.language as Language) || "python",
  );
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      await api.post(
        `/api/progress/${lesson.id}/complete?course_id=${lesson.course_id}`,
      );
      setIsCompleted(true);
  ...
  const handleRunCode = async (stdin: string) => {
    ...
    try {
      ...
      if (err instanceof HttpError) {
        setRunError(
          `Execution failed (${err.status}): ${err.body || err.message}`,
        );
      } else {
  ...
  const handleSubmitCode = async () => {
    ...
    if (allPassed) {
      ...
      setOutput(
        `Passed ${passedCount}/${lessonTestCases.length}\n\n` +
          resultLines.join("\n"),
      );
    }
    setIsRunning(false);
  ...
  useEffect(() => {
    const starter =
      (linkedQuestion?.starter as any)?.[language] ||
      lesson?.starter_code ||
      "";
  ...
  const resolvedTestCases =
    linkedQuestion?.test_cases || lesson.test_cases || [];
    ...
    <HydrationGuard>
      ...
      >
        ...
        <main className="flex-1 flex flex-col px-6 pt-6 pb-4 max-w-7xl mx-auto w-full min-h-0 overflow-hidden">
          ...
          {isExercise ? (
            <div className="flex-1 grid grid-cols-[40%_1fr] min-h-0 divide-x divide-white/[0.04]">
                  ...
                  <div className="mt-6 pt-5 border-t border-white/[0.04]">
                    ...
                    <div className="space-y-2">
                      {resolvedTestCases.map((tc, i) => (
                        <div
                          key={i}
                          className="border border-white/[0.04] rounded-lg px-3 py-2"
                        >
                          <p className="text-xs text-foreground/70 font-medium">
                            {tc.description}
                          </p>
                          <p className="text-[11px] font-mono text-muted-foreground/40 mt-1 tabular-nums">
                            input: &ldquo;{tc.input}&rdquo; &rarr; expected:
                            &ldquo;
