...
export function useCodeExecution(): CodeExecutionFeature {
  ...
  const validateCode = useCallback(
    async (language: string, code: string, testCases: TestCase[]) => {
      ...
      setOutput("");
      try {
        const result = await codeExecutionService.validateCode(
          language,
          code,
          testCases,
        );
  ...
  const submitCode = useCallback(
    async (
      ...
      try {
        ...
        setOutput(
          `Submit Results: ${result.passed_count}/${result.total} passed\n\n${outputLines.join("\n")}`,
        );
        setLastResult({
          total_tests: result.total,
          passed_tests: result.passed_count,
          success_rate:
            result.total > 0 ? result.passed_count / result.total : 0,
