    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl mx-4 p-1.5 rounded-[2rem] bg-white/[0.03] ring-1 ring-white/10">
        <div className="rounded-[calc(2rem-0.375rem)] bg-card shadow-[inset_0_1px_1px_rgba(255,255,255,0.08)] p-6">
          ...
          <div className="flex gap-0.5 mb-5 p-0.5 rounded-2xl bg-white/[0.03] ring-1 ring-white/5 w-fit">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={cn(
                  "px-4 py-1.5 text-xs font-medium rounded-xl transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
                  activeTab === tab.key
