...
export function QuestionDescriptionPanel({
  ...
  const hasFullData =
    "description" in selectedQuestion && selectedQuestion.description;

  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mb-5">
          <h2 className="text-base font-semibold text-foreground/90 mb-2 tracking-tight">
          ...
          <>
            ...
            {(selectedQuestion as Question).examples &&
              (selectedQuestion as Question).examples.length > 0 && (
                <div className="mb-5">
                  <h3 className="text-[10px] font-semibold tracking-wide text-muted-foreground/60 uppercase mb-3">
                    Examples:
                  </h3>
                  {(selectedQuestion as Question).examples.map(
                    (example, idx) => (
                      <div
                        key={idx}
                        className="mb-2 rounded-2xl bg-white/[0.03] ring-1 ring-white/5 p-1.5"
                      >
