...
describe("StructuredResponse", () => {
  ...
  it("renders hints as numbered list", () => {
    render(<StructuredResponse structured={validStructured} />);
    expect(
      screen.getByText("Try using a hash map for O(1) lookup"),
    ).toBeInTheDocument();
  ...
  it("renders suggestions as numbered list", () => {
    ...
    expect(
      screen.getByText("Use descriptive variable names"),
    ).toBeInTheDocument();
  });
  ...
  it("falls back to raw content when structured fails validation", () => {
    ...
    render(
      <StructuredResponse structured={invalid} rawContent="Fallback text" />,
    );
  ...
  it("returns null when no structured data and no raw content", () => {
    const { container } = render(
      <StructuredResponse
        structured={null as unknown as StructuredCoachingResponse}
      />,
    );
