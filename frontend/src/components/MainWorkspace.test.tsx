...
vi.mock('@/hooks', () => ({
  useLocalStorage: vi.fn((key: string, initial: Record<string, string>) => [initial, mockSetUserProgress]),
...
vi.mock('@/components/sidebar/Sidebar', () => ({
  Sidebar: vi.fn(({ questions, selectedQuestion, onSelectQuestion }: {
    questions: QuestionSummary[]; selectedQuestion: QuestionSummary | null;
    onSelectQuestion: (q: QuestionSummary) => void;
  }) => (
    <div data-testid="mock-sidebar" data-question-count={questions.length} data-has-selected={String(!!selectedQuestion)}>
      <button onClick={() => onSelectQuestion({ id: '1', title: 'Two Sum', difficulty: 'easy', category: 'arrays', company_tags: [] })}>
        Select Question
      </button>
    </div>
  )),
...
vi.mock('@/components/layout/elements', () => ({
  ...
  MainLayoutContainer: vi.fn(({ children }: { children: React.ReactNode }) => <div data-testid="main-layout">{children}</div>),
  MainContentContainer: vi.fn(({ children }: { children: React.ReactNode }) => <div data-testid="main-content">{children}</div>),
  ContentLayoutContainer: vi.fn(({ children }: { children: React.ReactNode }) => <div data-testid="content-layout">{children}</div>),
  QuestionContentSection: vi.fn(({ children }: { children: React.ReactNode }) => <div data-testid="question-section">{children}</div>),
  CodeEditorContainer: vi.fn(({ onRunCode, onSubmitCode, onCodeChange }: {
    onRunCode: () => void; onSubmitCode: () => void; onCodeChange: (c: string) => void;
  }) => (
  ...
  AIChatPanelContainer: vi.fn(({ onSendMessage, isTyping }: {
    onSendMessage: (msg: string, mode: string) => void; isTyping: boolean;
  }) => (
...
describe('MainWorkspace', () => {
  ...
  it('renders Sidebar with questions', () => {
    mockUseQuestion.mockImplementation(() => ({
      questions, selectedQuestion: null, fullQuestion: null,
      isLoading: false, isLoadingQuestion: false, error: null,
      loadQuestions: mockLoadQuestions, selectQuestion: mockSelectQuestion, clearError: mockClearError,
    }));
  ...
  it('calls sendMessage when question is selected', async () => {
    mockUseQuestion.mockImplementation(() => ({
      questions, selectedQuestion: questions[0], fullQuestion: null,
      isLoading: false, isLoadingQuestion: false, error: null,
      loadQuestions: mockLoadQuestions, selectQuestion: mockSelectQuestion, clearError: mockClearError,
    }));
    ...
    const user = userEvent.setup();
    await user.click(screen.getByText('Send Message'));
    expect(mockSendMessage).toHaveBeenCalledWith('hello', 'freeform', questions[0].title, '', 'python');
  });

  it('disables chat inputs when isTyping', () => {
    mockUseCoaching.mockImplementation(() => ({
      messages: [], isTyping: true, error: null,
      sendMessage: mockSendMessage, clearMessages: mockClearMessages, clearError: mockClearCoachingError,
    }));
  ...
  it('calls validateCode when Run Code is clicked with fullQuestion', async () => {
    const fullQuestion: Question = {
      id: '1', title: 'Two Sum', difficulty: 'easy', category: 'arrays', company_tags: [],
      description: 'test', examples: [], hints: [],
      starter: { python: '', javascript: '', java: '' },
      solution: '', time_complexity: '', space_complexity: '', test_cases: [],
    };
    mockUseQuestion.mockImplementation(() => ({
      questions, selectedQuestion: questions[0], fullQuestion,
      isLoading: false, isLoadingQuestion: false, error: null,
      loadQuestions: mockLoadQuestions, selectQuestion: mockSelectQuestion, clearError: mockClearError,
    }));
