...
export function CodeEditorContainer({
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newHeight = Math.max(
        100,
        Math.min(maxOutputHeight, startHeightRef.current + deltaY),
      );
  ...
  return (
    <div
      ref={containerRef}
      className="flex-1 flex flex-col overflow-hidden p-0 min-w-[300px]"
    >
      {isResizing && (
        <div className="fixed inset-0 z-[9999] cursor-row-resize bg-transparent select-none" />
      )}
      ...
      >
        ...
        {isInteractive && (
          <div className="bg-white/[0.02] border-t border-white/[0.04] p-3">
            <div className="text-xs font-medium tracking-wide text-muted-foreground/60 mb-2">
              INPUT (STDIN)
            </div>
            <textarea
      ...
      {hasOutput && !outputCollapsed && (
        ...
        >
          ...
          <GripVertical className="relative h-4 w-4 text-muted-foreground/40" />
        </div>
      )}
      ...
      <div
        ...
        style={
          hasOutput && !outputCollapsed
            ? { height: `${outputHeight}px`, flex: "0 0 auto" }
            : undefined
        }
      >
        {/* Output Header */}
