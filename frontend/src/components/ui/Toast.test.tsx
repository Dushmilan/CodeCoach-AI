...
describe('showToast', () => {
  it('renders a toast with the given message when ToastContainer is mounted', () => {
    ...
    act(() => { showToast('Hello world', 'info'); });
    expect(screen.getByText('Hello world')).toBeInTheDocument();
  });

  it('defaults to info variant', () => {
    render(<ToastContainer />);
    act(() => { showToast('Default variant'); });
...
describe('ToastContainer', () => {
  ...
  it('renders a toast when showToast is called', () => {
    render(<ToastContainer />);
    act(() => { showToast('Task completed', 'success'); });
  ...
  it('stacks multiple toasts', () => {
    render(<ToastContainer />);
    act(() => { showToast('Toast one', 'info'); });
    act(() => { showToast('Toast two', 'info'); });
    act(() => { showToast('Toast three', 'info'); });

    const container = screen.getByText('Toast one').closest('div[class*="fixed"]');
  ...
  it('auto-dismisses a toast after 4 seconds', () => {
    ...
    act(() => { showToast('Auto dismiss', 'info'); });
...
describe('ToastProvider', () => {
  ...
  it('renders toasts through the provider', () => {
    ...
    act(() => { showToast('Provider toast', 'success'); });
...
describe('useToast', () => {
  ...
  it('returned showToast renders a toast when container is mounted', () => {
    render(<ToastContainer />);
    const { result } = renderHook(() => useToast());
    act(() => { result.current.showToast('Via hook', 'info'); });
