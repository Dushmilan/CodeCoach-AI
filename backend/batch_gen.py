...
Include: title, difficulty, category (MUST be "{topic}"), description (2+ paragraphs), examples (2), test_cases (6 with input/expected_output/description/hidden), starter (python/javascript/java), hints (3), solution, time_complexity, space_complexity, constraints (4).
...
# Generate 1 question per topic per difficulty
total_generated = 0
for i, topic in enumerate(TOPICS):
    for difficulty in ['easy', 'medium', 'hard']:
        ...
        if total_generated % 5 == 0:
            ...
            print(f'  Saved checkpoint ({len(existing["questions"])} total)')
