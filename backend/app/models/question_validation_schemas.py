...
class ValidationIssue(BaseModel):
    ...
    use_case: ValidationUseCase = Field(
        ..., description="The use case that found this issue"
    )
    ...
    test_case_index: Optional[int] = Field(
        None, description="Test case index if applicable"
    )
...
class UseCaseValidationResult(BaseModel):
    ...
    use_case: ValidationUseCase = Field(
        ..., description="The use case that was validated"
    )
    ...
    execution_time_ms: Optional[float] = Field(
        None, description="Time taken to validate"
    )
...
class QuestionValidationResult(BaseModel):
    ...
    results: Dict[ValidationUseCase, UseCaseValidationResult] = Field(
        default_factory=dict, description="Results from each use case"
    )
...
class TestCaseValidationConfig(BaseModel):
    ...
    max_input_length: int = Field(
        default=10000, description="Maximum input length in characters"
    )
    max_output_length: int = Field(
        default=10000, description="Maximum output length in characters"
    )
    min_test_cases: int = Field(
        default=2, description="Minimum number of test cases required"
    )
    max_test_cases: int = Field(
        default=50, description="Maximum number of test cases allowed"
    )
    require_hidden_tests: bool = Field(
        default=False, description="Whether hidden tests are required"
    )
...
class TimeLimitConfig(BaseModel):
    ...
    default_time_limit_ms: int = Field(
        default=3000, description="Default time limit in milliseconds"
    )
    max_time_limit_ms: int = Field(
        default=10000, description="Maximum allowed time limit"
    )
    min_time_limit_ms: int = Field(
        default=100, description="Minimum allowed time limit"
    )
...
class FunctionSignatureConfig(BaseModel):
    ...
    require_type_hints: bool = Field(
        default=True, description="Whether type hints are required"
    )
...
class OutputFormatConfig(BaseModel):
    ...
    normalize_whitespace: bool = Field(
        default=True, description="Whether to normalize whitespace"
    )
    case_sensitive: bool = Field(
        default=False, description="Whether comparison is case sensitive"
    )
...
class QuestionValidationConfig(BaseModel):
    ...
    test_cases: TestCaseValidationConfig = Field(
        default_factory=TestCaseValidationConfig
    )
    time_limits: TimeLimitConfig = Field(default_factory=TimeLimitConfig)
    function_signature: FunctionSignatureConfig = Field(
        default_factory=FunctionSignatureConfig
    )
