...
class StructureValidationUseCase(BaseValidationUseCase):
    ...
    def _validate_id(self, id: str) -> List:
        ...
        if not id or not id.strip():
            issues.append(
                self._create_issue(message="Question ID cannot be empty", field="id")
            )
    ...
    def _validate_title(self, title: str) -> List:
        ...
        if not title or not title.strip():
            issues.append(
                self._create_issue(
                    message="Question title cannot be empty", field="title"
                )
            )
    ...
    def _validate_description(self, description: str) -> List:
        ...
        if not description or not description.strip():
            issues.append(
                self._create_issue(
                    message="Question description cannot be empty", field="description"
                )
            )
    ...
    def _validate_category(self, category: str) -> List:
        ...
        if not category or not category.strip():
            issues.append(
                self._create_issue(
                    message="Question category cannot be empty", field="category"
                )
            )
        return issues
    ...
    def _validate_difficulty(self, difficulty) -> List:
        ...
        if difficulty is None:
            issues.append(
                self._create_issue(
                    message="Difficulty level is required", field="difficulty"
                )
            )
        return issues
