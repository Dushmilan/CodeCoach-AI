...
class TestExecutionSecurity:
    ...
    def test_file_system_read_blocked(self, test_client: TestClient):
        ...
        for code in payloads:
            ...
            if response.status_code == 200:
                data = response.json()
                assert "root:" not in data.get("stdout", ""), f"File read leaked: {code}"
    ...
    def test_process_spawn_blocked(self, test_client: TestClient):
        ...
        for code in payloads:
            ...
            if response.status_code == 200:
                ...
                assert not any(name in stdout_lower for name in admin_names), f"Process spawn leaked: {code}"
    ...
    def test_js_require_fs(self, test_client: TestClient):
        ...
        response = test_client.post("/api/run/", json={
            "language": "javascript", "code": code, "stdin": ""
        })
        assert response.status_code in [200, 401]
        if response.status_code == 200:
            data = response.json()
