...
from app.services.auth_service import hash_password, create_access_token, TokenData, _get_secret_key
...
class TestAuthSecurity:
    ...
    def test_jwt_alg_none_rejected(self, test_client: TestClient):
        ...
        response = test_client.get("/api/auth/me", headers={"Authorization": f"Bearer {forged_token}"})
    ...
    def test_jwt_expired_token_rejected(self, test_client: TestClient):
        ...
        token, _ = create_access_token(expired_data, expires_delta=timedelta(seconds=-3600))
    ...
    def test_jwt_modified_claims_rejected(self, test_client: TestClient):
        ...
        response = test_client.get("/api/auth/me", headers={"Authorization": f"Bearer {tampered_token}"})
        assert response.status_code == 401
    ...
    def test_jwt_wrong_secret_rejected(self, test_client: TestClient):
        ...
        response = test_client.get("/api/auth/me", headers={"Authorization": f"Bearer {forged}"})
        assert response.status_code == 401
    ...
    def test_privilege_escalation_wrong_token(self, test_client: TestClient):
        ...
        response = test_client.get("/api/auth/me", headers={"Authorization": "Bearer random-invalid-token"})
        assert response.status_code == 401
    ...
    def test_register_duplicate_username(self, test_client: TestClient):
        ...
        payload = {"username": "dupuser", "email": "dup@test.com", "password": "StrongPass1!"}
