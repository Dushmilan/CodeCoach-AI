...
def test_array_bars_sorting_beats():
    spec = _spec("bars", [AnimationStepSpec(action="compare", indices=[0, 1]), AnimationStepSpec(action="swap", indices=[0, 1]), AnimationStepSpec(action="mark", index=4), AnimationStepSpec(action="window", low=0, high=2)], array=[5, 2, 8])
...
def test_array_dp_write_and_partition():
    spec = _spec("array", [AnimationStepSpec(action="write", index=2, values=[9]), AnimationStepSpec(action="partition", index=3)], array=[1, 2, 3])
...
def test_stack_push_pop():
    spec = _spec("stack", [AnimationStepSpec(action="push", values=[1]), AnimationStepSpec(action="push", values=[2]), AnimationStepSpec(action="pop", values=[2])])
...
def test_linked_list_visit():
    spec = _spec("linked_list", [AnimationStepSpec(action="visit", index=0), AnimationStepSpec(action="pointer", index=2)], array=[1, 2, 3, 4])
...
def test_tree_backtrack():
    spec = _spec("tree", [AnimationStepSpec(action="visit", index=0), AnimationStepSpec(action="choose", index=1), AnimationStepSpec(action="backtrack", index=1)])
...
def test_graph_edge_visit():
    spec = _spec("graph", [AnimationStepSpec(action="visit", index=0), AnimationStepSpec(action="edge", indices=[0, 1]), AnimationStepSpec(action="visit", index=1)])
...
def test_intervals():
    spec = _spec("intervals", [AnimationStepSpec(action="partition", indices=[0, 2]), AnimationStepSpec(action="visit", index=1)])
...
def test_backtrack_choose_backtrack():
    spec = _spec("backtrack", [AnimationStepSpec(action="choose", index=0), AnimationStepSpec(action="choose", index=1), AnimationStepSpec(action="backtrack", index=1)])
...
def test_dispatch_covers_all_visualizations():
    for viz in ["sorted-array", "bars", "array", "stack", "queue", "linked_list", "tree", "graph", "grid", "intervals", "backtrack"]:
